...
public class MapGenTutorialCaves extends MapGenBase
{
    ...
    {
    	// TADO RENAME ALL BLOCKS TO YOUR CUTOM BLOCKS AND STONE THATS USED UNDERGROUND
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        return (isExceptionBiome(biome) ? data[index] == (byte)Main.TutorialGrass.blockID : data[index] == biome.topBlock);
    }
    ...
    {
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        int top    = (isExceptionBiome(biome) ? (byte)Main.TutorialGrass.blockID : biome.topBlock);
        int filler = (isExceptionBiome(biome) ? (byte)Main.TutorialDirt.blockID  : biome.fillerBlock);
        int block  = data[index];

        if (block == (byte)Main.TutorialStone.blockID || block == filler || block == top)
        {
            if (y < 10)
            {
